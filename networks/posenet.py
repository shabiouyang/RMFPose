...
#sys.path.append('/home/datasets/.cache/hub/facebookresearch_dinov2_main/dinov2/')
# from models.vision_transformer import vit_small  
#from torchvision.models.vit_small
...
class GFObjectPose(nn.Module):
	...
	def __init__(self, cfg, prior_fn, marginal_prob_fn, sde_fn, sampling_eps, T):
		...
		if cfg.dino != 'none':
			...
			# print(torch.hub.load('/home/datasets/GenPose2/dinov2/dinov2', GFObjectPose.dino_name).to(cfg.device))
			# exit()

			# self.dino : nn.Module = torch.hub.load('facebookresearch/dinov2', GFObjectPose.dino_name)
			# with open('model_complete_dill.pth', 'wb') as f:
			# 	dill.dump(self.dino, f)
			#print(a, b)
			# with open('model_complete_dill.pth', 'rb') as f:
			# 	self.dino = dill.load(f)
			# exit()

			# self.dino : nn.Module = torch.hub.load('/home/datasets/.cache/hub/facebookresearch_dinov2_main/dinov2', \
			# 	'custom', path = '/home/datasets/.cache/hub/checkpoints/dinov2_vits14_pretrain.pth').to(cfg.device)

			# self.dino : nn.Module = vit_small(patch_size=14)
			# state_dict = torch.load('/home/datasets/.cache/hub/checkpoints/dinov2_vits14_pretrain.pth')
			# self.dino.load_state_dict(state_dict)
			# self.dino.to(cfg.device)
