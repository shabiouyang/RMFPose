...
def log_SO3(R):
    ...
    # 非原地操作构建新的 w_mat
    # new_w_mat = torch.zeros_like(R)
    # regular_part = torch.zeros_like(R)
    # regular_part[is_regular] = w_mat_regular
    # new_w_mat = regular_part

    # singular_part = torch.zeros_like(R)
    # singular_part[is_singular] = bracket_so3(w_vec_singular) * torch.pi
    # new_w_mat = torch.where(is_singular.unsqueeze(1).unsqueeze(2), singular_part, new_w_mat)
...
#     T[is_singular, :3, :3] = torch.eye(3).repeat(is_singular.sum(), 1, 1).to(S)

#     # print(T[is_singular, :3, 3].shape)
#     # print(p.shape)
#     # exit()
#     T[is_singular, :3, 3] = p[is_singular]

#     return T

def skew_batch(phi):
    """Batch 三维向量转反对称矩阵 [bs,3] -> [bs,3,3]"""
    batch_size = phi.size(0)
    zero = torch.zeros(batch_size, dtype=phi.dtype, device=phi.device)
    return torch.stack([
        zero, -phi[:,2], phi[:,1],
        phi[:,2], zero, -phi[:,0],
        -phi[:,1], phi[:,0], zero
...
def exp_SO3_batch(phi):
    """Batch SO3指数映射 [bs,3] -> [bs,3,3]"""
    theta = torch.norm(phi, dim=1, keepdim=True)
    theta = torch.clamp(theta, min=1e-8)
    phi_hat = skew_batch(phi)
    eye = torch.eye(3, dtype=phi.dtype, device=phi.device).unsqueeze(0)
    
    scale1 = torch.sin(theta) / theta
    scale2 = (1 - torch.cos(theta)) / theta**2
    
    R = eye + scale1.unsqueeze(-1)*phi_hat + scale2.unsqueeze(-1)*torch.matmul(phi_hat, phi_hat)
    return R

def exp_se3(xi):
    """Batch SE3指数映射 [bs,6] -> [bs,4,4]"""
    rho, phi = xi[:,:3], xi[:,3:]
    theta = torch.norm(phi, dim=1, keepdim=True)
    theta = torch.clamp(theta, min=1e-8)
    ...
    # 计算J矩阵
    phi_hat = skew_batch(phi)
    phi_hat_sq = torch.matmul(phi_hat, phi_hat)
    J = ( (1 - torch.cos(theta))/theta**2 ).unsqueeze(-1)*phi_hat + \
        ( (theta - torch.sin(theta))/theta**3 ).unsqueeze(-1)*phi_hat_sq
    J += torch.eye(3, device=xi.device).unsqueeze(0)
    
    # 计算平移
    t = torch.einsum('bij,bj->bi', J, rho)
    
    # 组合SE3矩阵
    T = torch.eye(4, device=xi.device).unsqueeze(0).repeat(xi.size(0),1,1)
    T[:,:3,:3] = R
    T[:,:3,3] = t
    return T
...
# def Lie_bracket(u, v):
#     """计算SE(3)李代数向量的李括号 [u, v]"""
#     # 转换输入为矩阵形式
#     u_mat = bracket_se3(u) if u.shape[-1] == 6 else bracket_so3(u)
#     v_mat = bracket_se3(v) if v.shape[-1] == 6 else bracket_so3(v)
    
#     # 计算矩阵交换子
#     bracket_matrix = u_mat @ v_mat - v_mat @ u_mat
    
#     # 转换回向量形式
#     return bracket_se3(bracket_matrix)  # 输出形状 [B,6]
...
def SE3_geodesic_dist(T_1, T_2):
    ...
    dist = (delta_R ** 2 + delta_p ** 2).sum(1)
