...
def aggregate_pose(score_path, energy_path, save_path):
    ...
    # print('all_pred_pose: ', len(all_pred_pose), len(all_pred_pose[0]), len(all_pred_pose[0][0]), len(all_pred_pose[0][0][0]))
    # print(all_pred_pose[0][0][0])
    # #print('all_pred_pose: ', len(all_pred_pose), len(all_pred_pose[0]), len(all_pred_pose[0][1]), len(all_pred_pose[0][1][0]))
    # exit()
    ...
    for i, (pred_pose, pred_energy) in enumerate(tqdm(zip(all_pred_pose, all_pred_energy), desc="aggregate")):
        ...
        #print(good_pose[0][0])
        rot_matrix = get_rot_matrix(good_pose[:, :, :-3].reshape(bs * retain_num, -1), cfg.pose_mode).reshape(bs, retain_num, 3, 3)
        ...
        aggregated_pose[:, :3, :3] = torch.mean(rot_matrix, dim=1).reshape(bs, 3, 3)
        aggregated_pose = aggregated_pose.permute(0, 2, 1)
...
def get_detect_match(cls_path, save_path):
    ...
    all_final_pose_b, all_final_length = pickle.load(open(cls_path, 'rb'))
    aggregate_path = '/home/datasets/GenPose2/results/evaluation_results/single_fm_full_4/aggregated.pkl'
    all_final_pose = pickle.load(open(aggregate_path, 'rb'))

    all_dm = []

    for i, test_batch in enumerate(tqdm(dataloader, desc="detect match")):
        #all_final_pose[i][:, :3, :3] = all_final_pose[i][:, :3, :3].permute(0, 2, 1)
        #pred_pose_b = all_final_pose_b[i].numpy()
        pred_pose = all_final_pose[i].numpy()
        #pred_pose[:, :3, 3] = pred_pose_b[:, :3, 3]
        ...
        #pred_pose[:, :3, 3] = (pred_pose[:, :3, 3]+gt_pose[:, :3, 3])/2
        # print('gt_pose: ', gt_pose[0])
        # print('gt_pose: ', gt_pose[1])
        # print('gt_pose: ', gt_pose[2])
        # print('gt_pose: ', gt_pose[3])
        # print('gt_pose: ', gt_pose[4])
        # # print('pred_pose: ', pred_pose[0])
        # #print('pred_pose_b: ', pred_pose_b[0])
        # # print('gt_length: ', gt_length[0])
        # # print('pred_length: ', pred_length[0])
        # exit()
        dm = DetectMatch(
            gt_affine=gt_pose, 
            gt_size=gt_length, 
            ...
            image_path=[path + 'color.jpg' for path in test_batch['path']],
...
def print_metrics(dm_path, criterion_path, save_path):
    ...
    #print("AUC @ IoU 75:", metrics.class_means.iou_auc[3].auc)
    ...
    # print("VUS @ 2 deg 1 cm:", metrics.class_means.pose_auc[4].auc)
    # print("VUS @ 2 deg 2 cm:", metrics.class_means.pose_auc[5].auc)
